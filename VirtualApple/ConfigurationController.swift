...
@MainActor
class ConfigurationController: NSViewController, NSTextFieldDelegate {
	...
	override func loadView() {
		...
		memorySlider = LabeledSlider(labels: memories.map {
		...
		let debugInformationalLabel = NSTextField(wrappingLabelWithString: """
		Starting a virtual machine with the GDB stub requires passing a check \
		(run by the VirtualMachine XPC service) against this process for the \
		com.apple.private.virtualization entitlement.
		""")
	...
	func validateUI() {
		...
		saveButton.isEnabled =
			Int(screenWidthTextField.stringValue) != nil &&
			Int(screenHeightTextField.stringValue) != nil &&
